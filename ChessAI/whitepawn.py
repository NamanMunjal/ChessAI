class whitepawn():
	...
	def checkMoves(self):
		...
		try:
			if self.board[int(self.ordinates[0])-1][int(self.ordinates[1])+1] != ' ' and self.board[int(self.ordinates[0])-1][int(self.ordinates[1])+1].startswith("B"):
				diagonalMove=True
		except:
			pass
		try:
